...
- `public/icons/` — PNG-иконки установки и Apple Touch Icon, полученные из `public/favicon.svg` скриптом `scripts/generate-icons.mjs` через Playwright в Docker. У maskable-иконки сплошной фон и отступ для системного обрезания. Скрипт принимает папку вывода (по умолчанию `/tmp/plukh-icons` внутри контейнера); готовые иконки хранятся в исходниках.
